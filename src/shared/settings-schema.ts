...
export const SETTINGS_DEFAULTS: SettingsMap = {
  ...
  night_dark_mode: false,
  night_screen_auto: false,
  night_screen_day: 100,
  night_dim_brightness: 75,
  night_brightness_entity: null,
  night_background_auto: false,
  night_background_hide: false,
  night_background_day: 100,
...
export const SETTINGS_FIELDS: SettingField[] = [
  ...
  { key: "vision_frame_count", label: "Images per pass", group: "Cameras", subsection: "Vision Analysis", globalOnly: true, kind: "number", min: 2, max: 8, help: "Total images sent to the AI per pass, including Frigate's labelled snapshot." },
  { key: "vision_frame_width", label: "Max image size", group: "Cameras", subsection: "Vision Analysis", globalOnly: true, kind: "number", min: 0, max: 4096, unit: "px", help: "Longest edge each image is downscaled to before the AI sees it (only downscales). 0 = full resolution." },
  { key: "vision_frame_adaptive", label: "Adaptive image count", group: "Cameras", subsection: "Vision Analysis", globalOnly: true, kind: "boolean", help: "Send fewer images on quiet events, scaled to how much the tracked object actually moved." },
  {
  ...
  { key: "night_mode", label: "Dynamic night mode", group: "General", kind: "nightmode", help: "On a nightly schedule, independently dim the screen, dim or hide the background, shift the font color, and switch to Dark Mode — each opt-in — restoring day values in the morning." },
  ...
  { key: "allow_device_area_self_assign", label: "Allow un-scoped devices to set their own Name / Area", group: "General", kind: "boolean", subsection: "Advanced", help: "Let a wall-panel (non-admin) account set its own device's name and room. A room can only be self-assigned when the device currently has no area; its name can be corrected later." },
