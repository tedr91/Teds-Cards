...
/** The light's own color when on (its rgb_color, else the themed on-foreground). */
function lightColor(stateObj: { attributes: { rgb_color?: number[] } }): string {
  const rgb = stateObj.attributes.rgb_color;
...
export class TedLightCard extends LitElement implements LovelaceCard {
  ...
  static styles = [
    ...
    /* On the HA theme (which may be a light theme) the dark-groove engraved look
       breaks, so use the active theme's own divider color as a flat hairline.
