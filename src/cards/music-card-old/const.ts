...
// Legacy wrapper card (droans/mass-player-card or YAMP). Preserved as a fallback while
// the native `ted-music-card` player is built; scheduled for deletion afterwards.
export const MUSIC_CARD_TYPE = `${NAMESPACE}-music-card-old`;
export const MUSIC_CARD_EDITOR_TYPE = `${MUSIC_CARD_TYPE}-editor`;
export const MUSIC_CARD_NAME = "Ted Music Card (Old)";
export const MUSIC_CARD_DESCRIPTION =
  "Legacy: drive a Music Assistant player UI (mass-player-card or yet-another-media-player) from this device's Settings music player.";
