...
export class TedCoverCardEditor extends LitElement implements LovelaceCardEditor {
  ...
  private _schema() {
    ...
    if (this._config?.show_hint !== false) {
      visual.push({
        name: "hint_width",
        selector: { number: { min: 0, max: 40, step: 1, mode: "box", unit_of_measurement: "px" } },
      });
  ...
  private _commit(raw: CoverCardConfig): void {
    const config = { ...raw } as CoverCardConfig & { mode?: string };
    ...
    if (config.icon_color === "theme" || config.icon_color === "other" || config.icon_color === "state") {
